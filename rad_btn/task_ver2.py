...
less_than_zero = "Длина стороны/сторон меньше нуля"
not_a_digit = "Длина стороны/сторон не является целым числом"
more_than_fifteen = "Длина стороны/сторон больше 15"
tit = "Ошибка данных"
...
def print_to_text():
    ...
    if int(variant.get()):
        ...
        try:
            ...
            messagebox.showerror(title=tit, message=not_a_digit)

    if width < 0 or height < 0:
        messagebox.showerror(title=tit, message=less_than_zero)
        0/0
    elif width > 15 or height > 15:
...
field = Text(root, width=30, height=15, state=DISABLED, font=glob_ft)
